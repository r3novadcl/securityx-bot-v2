...
const CATEGORIES = {
  ...
  Backup: {
    ...
    commands: [
      { name: "backup create",  desc: "Create a server backup" },
      { name: "backup load",    desc: "Restore a server backup" },
      { name: "backup delete",  desc: "Delete a backup" },
      { name: "backup list",    desc: "List all backups" },
    ],
  },
  Verification: {
    ...
    commands: [
      ...
      { name: "verification role",    desc: "Set the verified role" },
    ],
  },
  Logging: {
  ...
  General: {
    ...
    commands: [
      ...
      { name: "avatar", desc: "Fetch a user's avatar" },
...
module.exports = {
  ...
  async execute(message, args, client, prefix) {
    const sent = await message.channel.send({
      components: [buildMain(client, prefix)],
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.componentType === ComponentType.StringSelect)
        await sent.edit({ components: [buildCategory(i.client, i.values[0])], flags: MessageFlags.IsComponentsV2 });
      if (i.componentType === ComponentType.Button && i.customId === "help_back")
        await sent.edit({ components: [buildMain(i.client, prefix)], flags: MessageFlags.IsComponentsV2 });
    });
