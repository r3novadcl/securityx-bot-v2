...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    col.on('collect', async i => {
      await i.deferUpdate();
      if (i.customId === 'lockall_cancel') return msg.edit({ components: [simple('## Cancelled')], flags: MessageFlags.IsComponentsV2 });
      await msg.edit({ components: [simple(`## Locking...\n> Processing **${channels.size}** channels...`)], flags: MessageFlags.IsComponentsV2 });
      let done = 0;
