...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    col.on('collect', async i => {
      await i.deferUpdate();
      if (i.customId === 'uhall_cancel') return msg.edit({ components: [simple('## Cancelled')], flags: MessageFlags.IsComponentsV2 });
      let done = 0;
      for (const [, ch] of channels) {
        await ch.permissionOverwrites.edit(message.guild.roles.everyone, { ViewChannel: null }).catch(() => {});
        done++; await sleep(300);
      }
      await msg.edit({ components: [simple(`## Unhidden\n> **${done}** channels unhidden.`)], flags: MessageFlags.IsComponentsV2 });
    });
